  ...
  const [photoLink,setPhotoLink] = useState('')
    async function addPhotoByLink(e) {
      ...
      function removePhoto(e,filename){
        e.preventDefault()
        const updatedPhotos = existingPhotos.filter(file => file !== filename);
        onChange(updatedPhotos)
      }
      function makeMain(e,filename){
        e.preventDefault()
        onChange([filename,...existingPhotos.filter(file=>file !== filename)])
      }
  return (
    <>
            ...
            <div className='mt-2 grid grid-cols-3 lg:grid-cols-6 md:grid-cols-4'>
              {existingPhotos.length > 0 && existingPhotos.map(link=>(
                <div className='h-32 flex relative' key={link}>
                  <button onClick={(e) => makeMain(e,link)} className='cursor-pointer absolute top-3  left-4 bg-black bg-opacity-50 rounded-2xl text-white py-2 px-3 hover:bg-opacity-70'>
                    {link === existingPhotos[0] && (
                  ...
                  <button onClick={(e) => removePhoto(e,link)} className='cursor-pointer absolute bottom-3 right-4 bg-black bg-opacity-50 rounded-2xl text-white py-2 px-3 hover:bg-opacity-70'>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
